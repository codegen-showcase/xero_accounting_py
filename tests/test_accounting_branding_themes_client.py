...
def test_get_200_success_all_params() -> None:
    ...
    response = client.accounting.branding_themes.get(
...
async def test_await_get_200_success_all_params() -> None:
    ...
    response = await client.accounting.branding_themes.get(
        branding_theme_id="00000000-0000-0000-0000-000000000000",
        xero_tenant_id="YOUR_XERO_TENANT_ID",
    )
    try:
...
def test_list_200_success_all_params() -> None:
    ...
    # tests calling sync method with example data
    client = Client(oauth_token="API_TOKEN", environment=SIDEKO_MOCK_SERVER)
    response = client.accounting.branding_themes.list(
        xero_tenant_id="YOUR_XERO_TENANT_ID"
    )
    try:
        pydantic.TypeAdapter(models.BrandingThemes).validate_python(response)
...
async def test_await_list_200_success_all_params() -> None:
    ...
    # tests calling async method with example data
    client = AsyncClient(oauth_token="API_TOKEN", environment=SIDEKO_MOCK_SERVER)
    response = await client.accounting.branding_themes.list(
        xero_tenant_id="YOUR_XERO_TENANT_ID"
    )
    try:
        pydantic.TypeAdapter(models.BrandingThemes).validate_python(response)
