...
def test_create_200_success_all_params() -> None:
    """Tests a PUT request to the /Currencies endpoint.
    ...
    response = client.accounting.currencies.create(
        xero_tenant_id="YOUR_XERO_TENANT_ID", code="AUD", description="string"
    )
    try:
        pydantic.TypeAdapter(models.Currencies).validate_python(response)
...
async def test_await_create_200_success_all_params() -> None:
    ...
    response = await client.accounting.currencies.create(
        xero_tenant_id="YOUR_XERO_TENANT_ID", code="AUD", description="string"
    )
    try:
        pydantic.TypeAdapter(models.Currencies).validate_python(response)
...
def test_list_200_success_required_only() -> None:
    """Tests a GET request to the /Currencies endpoint.
    ...
    # tests calling sync method with example data
    client = Client(oauth_token="API_TOKEN", environment=SIDEKO_MOCK_SERVER)
    response = client.accounting.currencies.list(
...
async def test_await_list_200_success_required_only() -> None:
    ...
    # tests calling async method with example data
    client = AsyncClient(oauth_token="API_TOKEN", environment=SIDEKO_MOCK_SERVER)
    response = await client.accounting.currencies.list(
        xero_tenant_id="YOUR_XERO_TENANT_ID", order="Code ASC", where='Code=="USD"'
    )
    try:
        pydantic.TypeAdapter(models.Currencies).validate_python(response)
