...
res = client.accounting.currencies.list(
...
res = await client.accounting.currencies.list(
    xero_tenant_id="YOUR_XERO_TENANT_ID", order="Code ASC", where='Code=="USD"'
)
```
...
client = Client(oauth_token=getenv("API_TOKEN"))
res = client.accounting.currencies.create(
...
client = AsyncClient(oauth_token=getenv("API_TOKEN"))
res = await client.accounting.currencies.create(
    xero_tenant_id="YOUR_XERO_TENANT_ID", code="USD", description="United States Dollar"
)
```
